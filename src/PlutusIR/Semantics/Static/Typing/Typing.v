...
(* Fresh variable generated by appending ftvs. In case of no ftvs, "a" makes sure it is really fresh. *)
...
(* Helper with accumulator for drop_ty_var *)
...
(* Remove any type that has X free, or would be unshadowed by an earlier removed type (see thesis Richard Koetschruyter) *)
Definition drop_ty_var X (Γ : list (string * ty)) : list (string * ty) :=
  drop_ty_var' X Γ nil.

(* Helper to: remove type variables in bsn from Δ*)
Definition drop_Δ' (Δ : list (string * kind)) (bsn : list string) : list (string * kind) := filter (fun x => negb (inb_string (fst x) bsn)) Δ.

(* Remove type variables occuring in binders from Δ *)
Definition drop_Δ (Δ : list (string * kind)) (bs : list binding) : list (string * kind) := drop_Δ' Δ (btvbs bs).
